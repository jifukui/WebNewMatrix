...
uint8 CommandHandle(const char *sstr,json_t *json,json_t *ech,json_t *res,char *estr)
{
	json_error_t error;
    json_t *jsonget;
    jsonget=json_loads(sstr,JSON_ALLOW_NUL,&error);
    uint8 flag=0;
	int8 data[20000];
    if(jsonget)
    {
	...
	if((json_t*)0!=json&&JSON_OBJECT==json_typeof(json))
	{
		...
		if(JsonGetInteger(val,&type))
		{
			if(0==type)
			{
				...
				if((512==json_string_length(val))&&JsonGetString(val,edidstr))//(512==json_string_length(val)&&
				{
...
uint8 GetPortInfo(json_t * json,char *data,char* estr)
{
	...
	if(json&&(JSON_OBJECT==json_typeof(json))&&portinfo&&portsetting&&info&&arr&&cpy)
	{
		...
		if(JsonGetInteger(value,&portindex))
		{
			...
			if(status>=0)
			{
				...
				dir=lig_matrix_get_video_pipe_io(lighandle,portindex);
				...
				if(em_matrix_in==dir)
				{
					...
					if(!status)
					{
						if(in_info.signal_status>=0&&in_info.signal_status<=em_matrix_input_signal_dvi)
						{
							json_object_set_new(portinfo,"Singnal",json_string(ligsignalinfo[in_info.signal_status]));
						}
						else
						{
#if DEBUG
						...
						if(in_info.hdcp_version>=0&&in_info.hdcp_version<=em_matrix_HDCP_version_2_0)
						{
							json_object_set_new(portinfo,"HDCP",json_string(ligHDCPstatus[in_info.hdcp_version]));
						}
						else
						{
#if DEBUG
						...
						if(in_info.tmds_rates>=0&&in_info.tmds_rates<=em_matrix_signal_TMDS_rates_8K)
						{
						...
						if(in_info.cs_status>=0&&in_info.cs_status<=em_matrix_signal_cs_YUV420)
						{
							json_object_set_new(portinfo,"Color Speace",json_string(ligCSstatus[in_info.cs_status]));
						}
						else
						{
#if DEBUG
						...
						if(in_info.cd_status>=0&&in_info.cd_status<=em_matrix_signal_cd_48bit)
						{
				...
				else if(em_matrix_out==dir)
				{
					...
					if(!status)
					{
						if(out_info.display_status>=0&&out_info.display_status<=em_matrix_output_display_dvi)
						{
						...
						if(out_info.hdcp_version>=0&&out_info.hdcp_version<=em_matrix_HDCP_version_2_0)
						{
							json_object_set_new(portinfo,"HDCP",json_string(ligHDCPstatus[out_info.hdcp_version]));
						}
						else
						{
#if DEBUG
						...
						if(out_info.ssignal_ability>=0&&out_info.ssignal_ability<=em_matrix_ssignal_support_8K)
						{
						...
						if(out_info.cs_ability>=0&&out_info.cs_ability<=em_matrix_cs_support_YUV420)
						{
						...
						if(out_info.cd_ability>=0&&out_info.cd_ability<=em_matrix_cd_support_48bit)
						{
			...
			json_object_set_new(info,"Info",portinfo);
			...
			for(i=0;i<num;i++)
			{
				...
				if(!status)
				{
					...
					status=lig_matrix_get_port_parameter_value(lighandle,portindex,(EM_MATRIX_PARA_LIST)list[i],datalist,&datalen);
					if(!status)
					{
#if DEBUG
						printf("The name is %s\n",name);
			...
			json_object_set_new(info,"Setting",arr1);
			char *str;
...
uint8 _GetVideoOutputPortShapshot(uint32 output_port_id, char *data, char *estr)
{
    ...
    //Switch Speed
    status=lig_matrix_get_port_parameter_value(lighandle,output_port_id, 75, datalist, &datalen);
    if(!status)
    {
		...
		json_object_set(value,"Name",json_string("SwitchSpeed"));
...
uint8 _GetAudioPortShapshot(uint32 port_id, uint32 dir, char *data, char *estr)
{
    ...
    //未实现
    // //Audio Source
    // status=lig_matrix_get_port_parameter_value(lighandle,port_id, 98, datalist, &datalen);
    // printf("Audio Source : %d\n", status);
    // if(!status)
    // {
    //     int i = 0;
    //     printf("Audio Source is : ");
    //     for (i = 0; i < datalen; i++) {
    //         printf("%d ", datalist[i]);
    //     }
    //     // json_object_set(port_info,"Audio Source",json_integer(datalist[0]));
    // }
    // //Audio Source
    // //

    //ARC
    status=lig_matrix_get_port_parameter_value(lighandle,port_id, 102, datalist, &datalen);
    ...
    //Analog Audio: MUTE
    status=lig_matrix_get_port_parameter_value(lighandle,port_id, 91, datalist, &datalen);
    if(!status)
    {
		...
		json_object_set(value,"Name",json_string("MUTE"));
    ...
    //Analog Audio: Volume
    status=lig_matrix_get_port_parameter_value(lighandle,port_id, 81, datalist, &datalen);
    if(!status)
    {
    ...
    status=lig_matrix_get_port_parameter_value(lighandle,port_id, 84, datalist, &datalen);
    if(!status)
    {
        //json_object_set(analog_audio,"Balance",json_integer(datalist[0]));
		json_object_set(value,"Name",json_string("Balance"));
		json_object_set(value,"Value",json_integer(datalist[0]));
		cpy=json_deep_copy(value);
		json_array_append(arr,cpy);
    }
    ...
    //Analog Audio: Treble
    status=lig_matrix_get_port_parameter_value(lighandle,port_id, 88, datalist, &datalen);
    if(!status)
    {
        //json_object_set(analog_audio,"TREBLE",json_integer(datalist[0]));
		json_object_set(value,"Name",json_string("TREBLE"));
		json_object_set(value,"Value",json_integer(datalist[0]));
		cpy=json_deep_copy(value);
		json_array_append(arr,cpy);
    }
